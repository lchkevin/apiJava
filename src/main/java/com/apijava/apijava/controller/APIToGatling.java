...
public class APIToGatling {
    ...
    public ModelAndView process() {
        String systemName = "account_user";
        ...
        apiInfoList.forEach(e -> {
            ...
            result += "class " + e.getGatlingTestName() + " extends Simulation {\n";
            if ("accountfeed".equals("accountfeed")) {
            ...
            if (!e.getHeader().isEmpty()) {
                result += toHeaderTxtString(e.getHeader());
            }
            result += "    \"device_token\" -> \"297553f1036aaa40b9b0493df3f1f27e\",\n" +
                    "      \"Origin\" -> \"http://c6.web.com\")\n\n" +
                    "val scn = scenario(\"" + e.getGatlingTestName() + "\")\n" +
                    "    .during(testTimeSecs) {\n" +
                    "       feed(accounts)\n" +
                    ...
                    "          .resources(http(\"" + e.getGatlingTestName() + "\")\n" +
                    "               ." + e.getHttpMethod() + "(\"" + uri + "\")\n" +
                    "               .headers(headers_1)\n";
            if (!(e.getSetBody() == null)) {
                result += "               .body(StringBody(\"\"\"" + e.getSetBody() +"\"\"\"))\n";
            ...
            try {
                File file = new File("C:\\Users\\kevin\\workspace\\testGatling1\\src\\test\\scala\\" + e.getSystemName() + "\\" + e.getGatlingTestName() + ".scala");
                if (!file.exists()) {
